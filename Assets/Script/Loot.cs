...
public class Loot : MonoBehaviour
{
    ...
    [Range(0f, 1f)][SerializeField] private float bounceSfxVolume = 1f;
    [Tooltip("Minimum seconds between bounce SFX to avoid rapid double plays")]
    [SerializeField] private float bounceSfxMinInterval = 0.05f;
    [SerializeField] private bool playOnEveryBounce = true;
    [SerializeField] private bool playOnFinalBounce = false;

    // track last time bounce sfx played to debounce
    ...
    private void CheckGroundHit()
    {
        if (t_body.position.y < t_shadow.position.y)
        {
            ...
            {
                // play SFX for intermediate bounces if configured
                if (playOnEveryBounce)
                    TryPlayBounceSfx();
            ...
            {
                // final settle: optionally play final bounce SFX
                if (playOnFinalBounce)
                    TryPlayBounceSfx();
    ...
    // Safe bounce SFX play with debounce and AudioManager fallback
    private void TryPlayBounceSfx()
    {
        if (bounceSfx == null) return;

        if (Time.unscaledTime - lastBounceSfxTime < bounceSfxMinInterval) return;
        lastBounceSfxTime = Time.unscaledTime;

        if (AudioManager.Instance != null)
        {
            AudioManager.Instance.PlaySfx(bounceSfx, bounceSfxVolume);
            return;
        }
        ...
        src.clip = bounceSfx;
        src.volume = bounceSfxVolume;
        src.spatialBlend = 0f;
        src.Play();
        Destroy(src, bounceSfx.length + 0.1f);
    }
}
